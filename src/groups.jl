...
#         error("objects cannot be added to this group because it was created with a previous version of JLD3")
#     end
#     f = g.f
#     prewrite(f)
...
function load_group(f::JLDFile, roffset::RelOffset)
    ...
    while true
        ...
        while (curpos = position(cio)) < chunk_end # <= chunk_end-4
            if version == 1
