...
function constructrr(f::JLDFile, T::DataType, dt::CompoundDatatype,
    ...
    if !isconcretetype(T)
        ...
        return reconstruct_compound(f, T, dt, field_datatypes)
    end
    ...
    for i = 1:length(T.types)
        ...
        if !hasfielddata(writtenas)
            ...
            # if readtype <: TryConvert
            #     readtype = readtype.parameters[1]
            # end
...
struct TryConvert{T, INTERM} end

function reconstruct_compound(f::JLDFile, @nospecialize(T), dt::H5Datatype,
                              field_datatypes::Union{Vector{RelOffset},Nothing})
    rodr = reconstruct_odr(f, dt, field_datatypes)
    types = typeof(rodr).parameters[2].parameters

    # Now reconstruct the type
    reconname = gensym(string(T))
    # Core.eval(ReconstructedTypes,
    #           Expr(:struct, false, reconname,
    #                Expr(:block, Any[ Expr(Symbol("::"), dt.names[i], types[i]) for i = 1:length(types) ]...,
    #                     # suppress default constructors, plus a bogus `new()` call to make sure
    #                     # ninitialized is zero.
    #                     Expr(:if, false, Expr(:call, :new)))))
    # Tnew = getfield(ReconstructedTypes, reconname)
    Tnew = NamedTuple{tuple(dt.names...), Tuple{types...}}
    # if T isa DataType
    #     if hasmethod(T, Tuple{types...})
    #         @warn "Found a constructor with matching positional arguments. Will attempt to use that."
    #         return (ReadRepresentation{TryConvert{T, Tnew}, rodr}(), false)
    #     else
    #         @warn """You may try to implement a contructor with signature: 
    #         $T($(join(["$(dt.names[i])::$(types[i])" for i in 1:length(types)], ", ")))"""
    #     end
    # end
    return (ReadRepresentation{Tnew,rodr}(), false)
end

function jlconvert(  ::ReadRepresentation{TryConvert{T, INTERM}, ODR},
                    f::JLDFile,
                    ptr::Ptr,
                    header_offset::RelOffset) where {T, INTERM, ODR}
    rr = ReadRepresentation{INTERM, ODR}()
    interm = jlconvert(rr, f, ptr, header_offset)

    T(Any[getproperty(interm, p) for p in fieldnames(INTERM)]...)
end
