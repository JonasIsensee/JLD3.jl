...
"""
fileoffset(f::JLDFile, x::RelOffset) = Int64(x.offset + f.superblock.base_address)
...
"""
h5offset(f::JLDFile, x::Integer) = RelOffset(Int(x - f.superblock.base_address))
...
Opens a JLD3 file at path `fname`.
...
function Base.show(io::IO, ::MIME"text/plain", f::JLDFile)
    ...
    if !get(io, :compact, false)
        print(io, "\n")
        printtoc(io, f; numlines = get(io, :JLD3_numlines, 10))
    end
end
